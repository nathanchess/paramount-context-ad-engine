...
await put(\`analysis_v3_\${videoId}.json\`, JSON.stringify(parsed));`,
  },
  {
    ...
    description:
      "TwelveLabs' Marengo model generates 1024-dimensional embeddings for both video segments and ad creatives. Cosine similarity between these vectors provides scene-to-ad semantic matching that keyword matching cannot achieve.",
...
const whyRows = [
  {
    ...
    description:
      "State-of-the-art video representation model that encodes visual, audio, and textual content into a unified 1024-dimensional vector space. Powers semantic scene matching in this engine.",
  },
  {
    icon: (
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="px-8 pb-10 max-w-[1200px] mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { value: "39", label: "Ads in inventory", sub: "across 20 categories", icon: <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4"><rect x="2" y="3" width="12" height="10" rx="2" stroke="currentColor" strokeWidth="1.2" /><path d="M7 7l3 2-3 2V7Z" fill="currentColor" /></svg> },
            { value: "5", label: "Scene signals", sub: "per ad break", icon: <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4"><circle cx="8" cy="8" r="5.5" stroke="currentColor" strokeWidth="1.2" /><path d="M8 5v3l2 2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg> },
            { value: "1024", label: "Vector dimensions", sub: "Marengo embeddings", icon: <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4"><path d="M2 12L6 6l3 4 2-3 3 5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg> },
            { value: "3", label: "Safety modes", sub: "strict · balanced · max", icon: <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4"><path d="M8 2l5 2v4c0 3-2.5 5.5-5 6-2.5-.5-5-3-5-6V4l5-2z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" /></svg> },
          ].map((s) => (
      ...
      <section className="px-8 py-4 pb-12 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <FeatureCard
            ...
            icon={<svg viewBox="0 0 16 16" fill="none" className="w-5 h-5"><rect x="2" y="3" width="12" height="10" rx="2" stroke="currentColor" strokeWidth="1.2" /><path d="M7 7l3 2-3 2V7Z" fill="currentColor" /></svg>}
            title="Video Inventory Management"
            description="Upload broadcast episodes into the TwelveLabs index. Track duration, status, and analysis progress. HLS adaptive streaming included."
          />
          <FeatureCard
            iconBg="bg-purple-50 text-purple-600"
            icon={<svg viewBox="0 0 16 16" fill="none" className="w-5 h-5"><circle cx="8" cy="8" r="5.5" stroke="currentColor" strokeWidth="1.2" /><path d="M8 5v3l2 2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>}
            title="AI Scene Analysis"
            description="Pegasus generates time-stamped scene metadata — sentiment, tone, environment, suitable ad categories, GARM brand-safety flags, and interruption risk per segment."
          />
          <FeatureCard
